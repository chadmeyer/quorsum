...
\begin{equation}
   V = 2 T^2-T\box T
\end{equation}
where the box notation means element-by-element multiplication, i.e. if
$A=B\box C$ then $A_{ij}=B_{ij}C_{ij}$ rather than the normal matrix product.

If we sum across the first row of $V$, we will get the variance in the
mean total time to victory.  The square root of the variance is the 
root-mean-squared difference to the mean as expected.
